...
- **Farben** (Ansichtsoptionen): zwei unabhängige Regel-Listen — **Farbregeln** (Balkenfüllung) und **Textregeln** (Label-Stil). Jede Zeile ist entweder `property|wert|ausgabe` (greift, wenn die Notiz-Eigenschaft dem Wert entspricht; case-insensitive, Listen-Zugehörigkeit) oder nur eine `property` (der eigene Wert dieser Eigenschaft der Notiz ist die Ausgabe). Regeln werden von oben nach unten gelesen, **der erste Treffer gewinnt** pro Liste. Farb-Ausgabe ist ein Farbname, Hex oder `rgb(...)` (oder `none` für den Standard-Akzent); Text-Ausgabe ist eine beliebige Kombination aus `strike underline bold italic`, per Leerzeichen getrennt (oder `none` für normalen Text). Beispiel — Balken erledigter Aufgaben grau und durchgestrichen: `status|closed|gray` in den Farbregeln, `status|closed|strike` in den Textregeln.
