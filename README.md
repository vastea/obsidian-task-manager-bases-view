...
- **Colours** (view options): three independent rule lists — **Colour rules** (bar fill), **Text colour rules** (label colour) and **Text rules** (label style). Each line is either `property|value|output` (matches when the note's property equals the value; case‑insensitive, list membership) or a bare `property` (the note's own value of that property is the output). Rules are read top→bottom and the **first match wins** per list. Colour/text‑colour output is a colour name, hex, or `rgb(...)` (or `none` for the default); text output is any of `strike underline bold italic`, space‑separated (or `none` for plain). Example — bars for closed tasks grey and struck through: `status|closed|gray` in colours, `status|closed|strike` in text.
