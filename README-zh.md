...
- **属性标签**（可选）：开启**使用可见属性作为条目标签**，然后在 Bases 的**属性**菜单中有且只选择一个属性。其渲染值（支持 `file.*`、`note.*` 和公式属性）会同时替代左侧固定标签列与时间条中的文件名。空值或公式错误会原位显示警告；适合时可用 Bases 筛选排除这些条目。每个视图还可用**标签宽度**调整固定列。
- **泳道跟随 Bases 分组**（每组一条泳道），无分组时平铺。
- 起止齐全时显示条，只有一个日期时显示**里程碑**点。无日期项目默认隐藏；开启**包含无日期的条目**后，它们会作为临时里程碑显示在时间范围起点，拖动即可写入开始日期。
- 每格竖向网格线，方便看出条跨了几格；时间窗口在数据两侧留出约 3 个月/周的余量，并自动滚动到今天。
- **拖拽**整条平移双端，拖端点改单端（回写到 `note.*`）。
- **颜色**（视图选项）：三组独立规则——**颜色规则**（条填充）、**文本颜色规则**（标签颜色）与**文本规则**（标签样式）。每行是 `property|value|output`（当笔记属性等于该值时匹配；不区分大小写，支持列表成员）或仅一个 `property`（取该笔记此属性的自身值作为输出）。规则自上而下读取，每组**首个匹配生效**。颜色/文本颜色输出为颜色名、十六进制或 `rgb(...)`（或 `none` 表示默认）；文本输出为 `strike underline bold italic` 的任意组合，以空格分隔（或 `none` 表示普通文本）。示例——已关闭任务的条显示为灰色并加删除线：颜色规则 `status|closed|gray`，文本规则 `status|closed|strike`。
...
| 时间线 | `usePropertyDisplay` / `labelWidth` | 使用唯一一个 Bases 可见属性作为标签，并设置固定标签列宽度。 |
| 时间线 | `includeUndated` | 将默认隐藏的无日期项目显示为时间范围起点处可拖动的里程碑。 |
| 时间线 | `scale` | `day` / `week` / `month` / `quarter` / `year`。 |
| 时间线 | `rangePadding` | 项目前后的空白时间：`default` / `moderate` / `fit`。 |
| 时间线 | `autoZoom` | 根据面板宽度自动计算密度。 |
| 时间线 | `zoom` | 相对于默认刻度的显示密度。 |
| 时间线 | `ignoreMaxUnits` | 忽略全局最大单元格数并绘制完整范围。 |
| 时间线 | `colorRules` / `textColorRules` / `textRules` | 分别控制条填充、标签颜色和标签样式的首个匹配规则。 |
