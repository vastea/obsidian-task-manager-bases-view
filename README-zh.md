...
- **颜色**（视图选项）：两组独立规则——**颜色规则**（条填充）与**文本规则**（标签样式）。每行是 `property|value|output`（当笔记属性等于该值时匹配；不区分大小写，支持列表成员）或仅一个 `property`（取该笔记此属性的自身值作为输出）。规则自上而下读取，每组**首个匹配生效**。颜色输出为颜色名、十六进制或 `rgb(...)`（或 `none` 表示默认强调色）；文本输出为 `strike underline bold italic` 的任意组合，以空格分隔（或 `none` 表示普通文本）。示例——已关闭任务的条显示为灰色并加删除线：颜色规则 `status|closed|gray`，文本规则 `status|closed|strike`。
