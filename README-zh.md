...
![看板：预定义列（backlog / todo / doing / review / done）各带柔和底色；done 列带一键归档按钮，空的「未分组」列折叠成右侧竖条](docs/images/kanban-1.png)
...
- **重叠时间块**并排显示。在空白处**拖拽**（上下皆可）通过弹窗新建（描述 + 可选 `[[任务]]` 链接 + 类型）。**右键**时间块删除。单击打开当天日记并定位到该行。
...
3. 日历：执行命令 **打开周历日志**（或点侧栏时钟图标）。
4. 全局、跨文件的约定（日记文件夹、日期格式、每周起始、可见时段、日志章节、类型表、回写）在 **设置 → Task Manager Bases View** 里。

> 在设置里开关某个视图后，需要在「第三方插件」里**关闭再启用本插件**（或重启 Obsidian）才会生效——Bases 没有提供单视图注销 API。

每个视图可独立开关；日历的相关设置仅在其视图启用时才显示，保持设置页清爽：

![设置——「周历日志日历」区（因其视图开关已开启而显示）：日记文件夹、日期格式、每周起始、可见时段（开始 / 结束）下拉、日志章节标题、可选回写、按类型着色的分类表](docs/images/settings-2.png)
